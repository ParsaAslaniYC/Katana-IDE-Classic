...
#
# Copyright (C) 1996-1998 Stewart Allen, 2020-2021 SayedAli Nourian Najafabadi & Donald Rozenberg
#
...
proc vTclWindow.vTcl {args} {
    ...
    # wm title $vTcl(gui,main) "SEEMAN"  ;# Rozen
    wm title .vTcl "SEEMAN - $vTcl(project,name)"
    wm resizable $vTcl(gui,main) 0 0
    wm group $vTcl(gui,main) $vTcl(gui,main)
    wm command $vTcl(gui,main) "$vTcl(VTCL_HOME)/page"
    wm iconname $vTcl(gui,main) "SEEMAN"   ;# Rozen
...
proc vTcl:display_help {} {
    # Display SEEMAN options. When closed, SEEMAN exits.
    global vTcl
    toplevel .top
    wm title .top "Help"
    text .top.t -width 70 -height 13
    pack .top.t
    .top.t insert 1.0 "SEEMAN version: $vTcl(version)\n\n"
...
proc vTcl:main {argc argv} {
    ...
    update
    if {![info exists env(SEEMAN_HOME)]} {
        set home [file dirname [info script]]
        switch [file pathtype $home] {
            absolute {set env(SEEMAN_HOME) $home}
            relative {set env(SEEMAN_HOME) [file join [pwd] $home]}
            volumerelative {
                set curdir [pwd]
                cd $home
                set env(SEEMAN_HOME) [file join [pwd] [file dirname \
    ...
    set vTcl(VTCL_HOME) $env(SEEMAN_HOME)

    # Stuff to support Img. I got these image packages from
    # Sourceforge and this is a hack to relieve the SEEMAN user from
    ...
    if {![file isdir $env(SEEMAN_HOME)]} { set vTcl(VTCL_HOME) [pwd] }
    ...
    vTcl:splash_status "              SEEMAN Loaded" -nodots
...
#set s_class [vTcl:get_class $s_target]
#        if {$s_target ne ""} {}
